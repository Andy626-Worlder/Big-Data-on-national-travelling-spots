                ...
                if j.get('mainTicket', False):
                    ...
                    if subTickets:
                        for sub in subTickets:
                            if not sub.get('subTicketGroupInfo') or not sub['subTicketGroupInfo'].get(
                                    'priceInfo') or not sub['subTicketGroupInfo']['priceInfo'].get('price'):
                                continue
...
def CrawlDataByCity(city):


    f = open(f'data/{city}.csv', 'w', encoding='utf-8', newline='')
        ...
        for poi in poiList:
            ...
            if isCrawlComment == 1:  # 爬取评论
                try:
                    ...
                    GetComments(ID, commentCount, poi.get('name'))
...
if __name__ == '__main__':
    ...
    citylist = ['厦门', '北京', '香港', '丽江', '三亚', '成都', '上海', '大理', '大理', '西安']
    for city in citylist:
        data['districtId'] = getCityID(city)  # 获取城市编号
        CrawlDataByCity(city)
